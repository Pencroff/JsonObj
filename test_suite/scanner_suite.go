...
type ScannerTestElement struct {
	idx  string
	in   []byte
...
func (s *ScannerTestSuite) TestScanner_Next_null() {
	tbl := []ScannerTestElement{
		{"null:0", []byte("null"), djs.ScanNull, []byte("null"), nil},
		{"null:1", []byte("null "), djs.ScanNull, []byte("null"), nil},
		{"null:2", []byte("null\n"), djs.ScanNull, []byte("null"), nil},
		{"null:3", []byte("null\r"), djs.ScanNull, []byte("null"), nil},
		{"null:4", []byte("null\t"), djs.ScanNull, []byte("null"), nil},
		{"null:5", []byte("null\r\n"), djs.ScanNull, []byte("null"), nil},
		{"null:6", []byte("null\t\n"), djs.ScanNull, []byte("null"), nil},
		{"null:7", []byte("null\t\r"), djs.ScanNull, []byte("null"), nil},
		{"null:8", []byte(" null "), djs.ScanNull, []byte("null"), nil},
		{"null:9", []byte(" null\n"), djs.ScanNull, []byte("null"), nil},
		{"null:10", []byte("n"), djs.ScanUnknown, []byte(nil), djs.InvalidJsonError},
		{"null:10", []byte("nnn"), djs.ScanUnknown, []byte(nil), djs.InvalidJsonError},
	}
	for _, el := range tbl {
		RunTest(el, s)
...
//		{"str:04", []byte(`"abc/xyz"`), djs.ScanString, []byte(`abc\bxyz`)},
...
func RunTest(el ScannerTestElement, s *ScannerTestSuite) {
	s.T().Run(el.idx, func(t *testing.T) {
		b := bytes.NewBuffer(el.in)
		bf := bufio.NewReader(b)
		sc := djs.NewJStructScanner(bf)
