...
func (s *TokenizerTestSuite) TestTokenizer_Next_string() {
	testCases := []TokenizerTestElement{
		...
		{"str:07", []byte(`"a\"z"`), djs.TokenString, []byte{0x61, 0x5c, 0x22, 0x7a}, nil},
		...
		{"str:23", []byte(`"D'fhuascail Íosa, Úrmhac na hÓighe Beannaithe, pór Éava agus Ádhaimh"`), djs.TokenString, []byte(`D'fhuascail Íosa, Úrmhac na hÓighe Beannaithe, pór Éava agus Ádhaimh`), nil},
		{"str:24", []byte(`"いろはにほへとちりぬるを"`), djs.TokenString, []byte(`いろはにほへとちりぬるを`), nil},
